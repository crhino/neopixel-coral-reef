...
import board
import neopixel
from adafruit_led_animation.animation.color import CYAN, BLUE, AQUA, GREEN
...
def color_cycle_array(start, end, steps):
    list(map(lambda step : interpolate_color_between(start, end, step/steps), itertools.count(steps)))
